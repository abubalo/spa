...
const router = async () => {
  ...
  const routes = [
    ...
    { path: "/posts", view: () => import("./views/Posts.js") },
    { path: "/posts/:id", view: () => import("./views/PostView.js") },
  ...
  const loadViewWithTimeout = async (viewLoader) => {
    return Promise.race([
      viewLoader(),
      new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error("View loading timed out!")),
          TIMEOUT_DURATION
        )
      ),
    ]);
